...
const helpMessages = new Map<string, string>([
    ...
    ['ANIMATION', 'ANIMATION: 创建动画.\n- 必须参数: name, code (执行的代码块), slot (更新的槽位).\n- 可选参数: interval (毫秒), repeat (布尔值).'],
]);

const metaCommands = ['CLEAR', 'SET', 'HELP', 'VIEW', 'TRANSLATE', 'DRAW', 'MEASURE', 'RUN', 'CODE', 'WITH', 'CALCULATE', 'GETOBJ', 'PRINT', 'CREATE'];
