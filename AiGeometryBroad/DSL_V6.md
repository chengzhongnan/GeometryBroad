...
* `SET item=<item> value=<value>`
    ...
    * **可用 item**:
        ...
        * `penSize`: 默认画笔粗细。
        * `labelFont` / `labelColor` / `labelSize`: 标签字体、颜色、字号。
        * `pointRadius` / `pointFill`: 默认点半径、是否填充。
        * `drawLabelForPoints` / `drawLabelForOthers` / `drawAfterCreate`: 是否自动绘制标签/创建对象后自动绘制。
        * `centerX` / `centerY` / `scale`: 视图中心与缩放。
...
* **2.5.3 `CREATE TRIANGLE`**: `name=<name> p1=<p1> p2=<p2> p3=<p3>`
    * **描述**: 通过三个顶点创建一个**三角形**。
